function [ph,ph_error,mse]=LSCDE(x_train,y_train,x_test,y_test,w)
%
...
% Modified to accomodate weights 
...
b=min(100,n); % Number of kernel bases
  ...
  for sigma_index=1:length(sigma_list)
    ...
    phi_zw=W.*phi_xu.*phi_yv;%h
    phi_vv=exp(-vv_dist2/(4*sigma^2));
    for k=1:fold
      tmp=phi_xu(:,cv_index(cv_split==k));tmp1=w(1,cv_index(cv_split==k));W1=repmat(tmp1,[b,1]);
      Phibar_cv(:,:,k)=(sqrt(pi)*sigma)^d_y*phi_vv.*((tmp.*W1)*tmp');
    end % for fold
  ...
  %%%%%%%%%%% MSE %%%%%%%%%
 mse=0.5*alphah'*Phibar/n*alphah-mean(phi_zw,2)'*alphah;
